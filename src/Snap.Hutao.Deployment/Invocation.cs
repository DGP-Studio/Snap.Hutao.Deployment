...
internal static partial class Invocation
{
    public static async Task RunDeploymentAsync(InvocationContext context)
    {
        ...
        Console.WriteLine($"""
            Snap Hutao Deployment Tool [1.16.6]
            PackagePath: {path}
            FamilyName: {name}
            ------------------------------------------------------------
            """);
        ...
        if (result.IsRegistered)
        {
            ...
            if (string.IsNullOrEmpty(name))
            {
                ...
                foreach (Windows.ApplicationModel.Package package in packageManager.FindPackages())
                {
                    ...
                    {
                        // ERROR_MRM_MAP_NOT_FOUND or ERROR_MRM_NO_MATCH_OR_DEFAULT_CANDIDATE or ERROR_NOT_FOUND
                        if (ex.HResult is not (unchecked((int)0x80073B1F) or unchecked((int)0x80073B0C) or unchecked((int)0x80070490)))
                        {
                            throw;
                        }
                    }
